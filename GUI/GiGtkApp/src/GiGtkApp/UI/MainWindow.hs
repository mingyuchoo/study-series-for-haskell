...
appendToBox :: (GObject child, IsDescendantOf Gtk.Widget child) => Gtk.Box -> child -> IO ()
appendToBox parent child = do
    widget <- Gtk.toWidget child
    #append parent widget

appendToListBox :: (GObject child, IsDescendantOf Gtk.Widget child) => Gtk.ListBox -> child -> IO ()
