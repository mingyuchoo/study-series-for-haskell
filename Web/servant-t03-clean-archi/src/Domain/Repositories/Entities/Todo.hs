...
data Priority -- | Low priority tasks
              = Low
...
mkTodo todoId' title time prio stat =
  Todo
    { todoId = todoId'
    , todoTitle = title
    , createdAt = time
...
validateTodoTitle title
  ...
  | T.length title < 3 =
      Left <| ValidationError "TodoTitle must be at least 3 characters long"
  | T.length title > 50 =
      Left <| ValidationError "TodoTitle must be at most 50 characters long"
  | otherwise = Right ()
