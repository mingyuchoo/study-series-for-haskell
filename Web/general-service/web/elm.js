...
var $author$project$Application$TaskBoard$addEffect = F2(
	function (effect, _v0) {
		var model = _v0.a;
		var effects = _v0.b;
...
var $author$project$Domain$Task$importanceFromString = function (rawImportance) {
	switch (rawImportance) {
		case 'Important':
			return $elm$core$Maybe$Just($author$project$Domain$Task$Important);
		case 'NotImportant':
			return $elm$core$Maybe$Just($author$project$Domain$Task$NotImportant);
		default:
			return $elm$core$Maybe$Nothing;
	}
};
...
var $author$project$Application$TaskBoard$replaceTask = F2(
	function (movedTask, currentTask) {
		return _Utils_eq(currentTask.taskId, movedTask.taskId) ? movedTask : currentTask;
	});
var $author$project$Application$TaskBoard$ClearNoticeAfter = function (a) {
	return {$: 'ClearNoticeAfter', a: a};
};
var $author$project$Application$TaskBoard$showNotice = F2(
	function (message, model) {
		var nextVersion = model.noticeVersion + 1;
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
			...
			_List_fromArray(
				[
					$author$project$Application$TaskBoard$ClearNoticeAfter(nextVersion)
				]));
	});
var $author$project$Domain$Task$Approved = {$: 'Approved'};
var $author$project$Domain$Task$Effective = {$: 'Effective'};
var $author$project$Domain$Task$Reviewed = {$: 'Reviewed'};
var $author$project$Domain$Task$Submitted = {$: 'Submitted'};
var $author$project$Domain$Task$statusFromString = function (rawStatus) {
	switch (rawStatus) {
		case 'Draft':
			return $elm$core$Maybe$Just($author$project$Domain$Task$Draft);
		case 'Reviewed':
			return $elm$core$Maybe$Just($author$project$Domain$Task$Reviewed);
		case 'Submitted':
			return $elm$core$Maybe$Just($author$project$Domain$Task$Submitted);
		case 'Approved':
			return $elm$core$Maybe$Just($author$project$Domain$Task$Approved);
		case 'Effective':
			return $elm$core$Maybe$Just($author$project$Domain$Task$Effective);
		default:
			return $elm$core$Maybe$Nothing;
	}
};
var $author$project$Domain$Task$statusLabel = function (taskStatus) {
	switch (taskStatus.$) {
		case 'Draft':
			return '초안';
		case 'Reviewed':
...
var $author$project$Application$TaskBoard$toInput = function (task) {
	return {description: task.description, expectedResult: task.expectedResult, importance: task.importance, outcomeOwner: task.outcomeOwner, status: task.status, taskOwner: task.taskOwner, title: task.title, urgency: task.urgency};
};
var $author$project$Application$TaskBoard$updateForm = F2(
...
var $author$project$Domain$Task$Urgent = {$: 'Urgent'};
var $author$project$Domain$Task$urgencyFromString = function (rawUrgency) {
	switch (rawUrgency) {
		case 'Urgent':
			return $elm$core$Maybe$Just($author$project$Domain$Task$Urgent);
		case 'NotUrgent':
			return $elm$core$Maybe$Just($author$project$Domain$Task$NotUrgent);
		default:
			return $elm$core$Maybe$Nothing;
	}
};
		...
		switch (msg.$) {
			...
			case 'EditStatus':
				...
				if (_v9.$ === 'Just') {
					...
					return A2($author$project$Application$TaskBoard$showNotice, '알 수 없는 업무 상태입니다.', model);
				}
			case 'EditUrgency':
				...
				if (_v10.$ === 'Just') {
					...
					return A2($author$project$Application$TaskBoard$showNotice, '알 수 없는 긴급도입니다.', model);
				}
			case 'EditImportance':
				...
				if (_v11.$ === 'Just') {
					...
					return A2($author$project$Application$TaskBoard$showNotice, '알 수 없는 중요도입니다.', model);
				}
			case 'EditTaskOwner':
				var owner = msg.a;
				return _Utils_Tuple2(
				...
				if (_v12.$ === 'Err') {
					...
					if (_v14.$ === 'Just') {
						...
						return _Utils_Tuple2(
							...
							_List_fromArray(
								[
									A2($author$project$Application$TaskBoard$UpdateTask, task.taskId, input)
								]));
					} else {
						return _Utils_Tuple2(
							_Utils_update(
							...
							_List_fromArray(
								[
									$author$project$Application$TaskBoard$CreateTask(
									_Utils_update(
										input,
										{status: $author$project$Domain$Task$Draft}))
								]));
					}
				}
			case 'StartEdit':
			...
			case 'Saved':
				...
				if (result.$ === 'Ok') {
					return A2(
						$author$project$Application$TaskBoard$addEffect,
						$author$project$Application$TaskBoard$LoadTasks,
						A2(
							$author$project$Application$TaskBoard$showNotice,
							'업무가 저장되었습니다.',
			...
			case 'DragStarted':
				var taskId = msg.a;
				return model.loading ? _Utils_Tuple2(model, _List_Nil) : _Utils_Tuple2(
					_Utils_update(
						model,
						{
							draggedTaskId: $elm$core$Maybe$Just(taskId),
				...
				if (_v17.$ === 'Just') {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								dropTarget: $elm$core$Maybe$Just(taskStatus)
							}),
						_List_Nil);
				} else {
					return _Utils_Tuple2(model, _List_Nil);
			...
			case 'DragEnded':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{draggedTaskId: $elm$core$Maybe$Nothing, dropTarget: $elm$core$Maybe$Nothing}),
					_List_Nil);
			case 'DroppedOn':
				...
				if ((!_v18.a) && (_v18.b.$ === 'Just')) {
					...
					if (_v19.$ === 'Just') {
						...
						return _Utils_eq(task.status, targetStatus) ? _Utils_Tuple2(
							...
							_Utils_update(
								model,
								{draggedTaskId: $elm$core$Maybe$Nothing, dropTarget: $elm$core$Maybe$Nothing, loading: true, notice: $elm$core$Maybe$Nothing}),
							_List_fromArray(
								[
									A2(
									$author$project$Application$TaskBoard$MoveTask,
									taskId,
									{description: task.description, expectedResult: task.expectedResult, importance: task.importance, outcomeOwner: task.outcomeOwner, status: targetStatus, taskOwner: task.taskOwner, title: task.title, urgency: task.urgency})
								]));
					} else {
						return _Utils_Tuple2(
							_Utils_update(
			...
			case 'MoveSaved':
				var result = msg.a;
				if (result.$ === 'Ok') {
					var movedTask = result.a;
					return A2(
						$author$project$Application$TaskBoard$showNotice,
						'업무 상태를 ' + ($author$project$Domain$Task$statusLabel(movedTask.status) + ' 상태로 변경했습니다.'),
						_Utils_update(
							model,
							{
								loading: false,
								tasks: A2(
									$elm$core$List$map,
									$author$project$Application$TaskBoard$replaceTask(movedTask),
									model.tasks)
							}));
				} else {
					var error = result.a;
			...
			default:
				var version = msg.a;
				return _Utils_eq(model.noticeVersion, version) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{notice: $elm$core$Maybe$Nothing}),
...
var $author$project$Presentation$TaskBoard$radioField = F6(
	function (fieldName, fieldLabel, selectedValue, choices, toString, toMessage) {
		return A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('field radio-field')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							...
							A2($elm$html$Html$Attributes$attribute, 'aria-label', fieldLabel)
						]),
					A2(
						$elm$core$List$map,
						function (_v0) {
							var choice = _v0.a;
							var choiceLabel = _v0.b;
							return A2(
								$elm$html$Html$label,
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class('radio-option')
									]),
								_List_fromArray(
									[
										A2(
										...
										_List_fromArray(
											[
												...
												_Utils_eq(selectedValue, choice)),
												$elm$html$Html$Events$onClick(
												toMessage(
													toString(choice)))
											]),
										...
										_List_fromArray(
											[
												$elm$html$Html$text(choiceLabel)
											]))
									]));
						},
						choices))
	...
	function (selectedStatus, current) {
		return A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$Attributes$value(
					$author$project$Domain$Task$statusString(current)),
					$elm$html$Html$Attributes$selected(
					_Utils_eq(selectedStatus, current))
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(
...
var $author$project$Presentation$TaskBoard$formView = function (model) {
	var isEditing = !_Utils_eq(model.editing, $elm$core$Maybe$Nothing);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('card editor')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class(
								'field title-field' + (isEditing ? '' : ' full-width'))
							]),
						_List_fromArray(
							[
								A2(
								...
								_List_fromArray(
									[
										...
										$elm$html$Html$Attributes$value(model.draft.title),
										$elm$html$Html$Attributes$placeholder('예: 2026년 4분기 계약 갱신'),
										$elm$html$Html$Events$onInput($author$project$Application$TaskBoard$EditTitle)
									]),
								_List_Nil)
							])),
						isEditing ? A2(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$for('status')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text('현재 상태')
									])),
								A2(
								...
								_List_fromArray(
									[
										...
										$elm$html$Html$Events$onInput($author$project$Application$TaskBoard$EditStatus)
									]),
								A2(
									$elm$core$List$map,
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('field description-field')
							]),
						_List_fromArray(
							[
								A2(
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$for('expected-result')
									]),
								_List_fromArray(
									[
										$elm$html$Html$text('기대 결과물')
									])),
								A2(
								$elm$html$Html$textarea,
								_List_fromArray(
									[
										$elm$html$Html$Attributes$id('expected-result'),
										$elm$html$Html$Attributes$value(model.draft.expectedResult),
										$elm$html$Html$Attributes$placeholder('Outcome Owner가 원하는 결과물을 정의하세요.'),
										$elm$html$Html$Events$onInput($author$project$Application$TaskBoard$EditExpectedResult)
									]),
								_List_Nil)
							])),
						A2(
							...
							[
								...
								_List_fromArray(
									[
										...
										$elm$html$Html$Attributes$value(model.draft.description),
										$elm$html$Html$Attributes$placeholder('관리자 메모 또는 업무의 다음 단계를 적어주세요.'),
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('brand-row')
					]),
				_List_fromArray(
					[
						A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('업무의 흐름을 한눈에 보고, 다음 단계로 자연스럽게 이어가세요.')
					]))
			]));
};
...
var $author$project$Application$TaskBoard$DragOver = function (a) {
	return {$: 'DragOver', a: a};
};
var $author$project$Application$TaskBoard$DroppedOn = function (a) {
	return {$: 'DroppedOn', a: a};
};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$elm$html$Html$Attributes$attribute,
					'draggable',
					model.loading ? 'false' : 'true'),
					A2(
					$elm$html$Html$Events$on,
					'dragstart',
					$elm$json$Json$Decode$succeed(
						$author$project$Application$TaskBoard$DragStarted(task.taskId))),
					A2(
					$elm$html$Html$Events$on,
					'dragend',
					$elm$json$Json$Decode$succeed($author$project$Application$TaskBoard$DragEnded))
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('row-actions')
								]),
							_List_fromArray(
								[
									A2(
...
var $author$project$Presentation$TaskBoard$kanbanColumn = F2(
	function (model, taskStatus) {
		var statusTasks = A2(
			$elm$core$List$filter,
			function (task) {
				return _Utils_eq(task.status, taskStatus);
			},
			model.tasks);
		var dropTargetClass = _Utils_eq(
			model.dropTarget,
			$elm$core$Maybe$Just(taskStatus)) ? ' is-drop-target' : '';
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class(
					'kanban-column ' + ($author$project$Domain$Task$statusClass(taskStatus) + dropTargetClass)),
					A2(
					$elm$html$Html$Events$preventDefaultOn,
					'dragover',
					$elm$json$Json$Decode$succeed(
						_Utils_Tuple2(
							$author$project$Application$TaskBoard$DragOver(taskStatus),
							true))),
					A2(
					$elm$html$Html$Events$preventDefaultOn,
					'drop',
					$elm$json$Json$Decode$succeed(
						_Utils_Tuple2(
							$author$project$Application$TaskBoard$DroppedOn(taskStatus),
							true)))
				]),
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('column-heading')
						]),
					_List_fromArray(
						[
							A2(
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$author$project$Domain$Task$statusLabel(taskStatus))
								])),
							A2(
							$elm$html$Html$span,
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('column-count')
								]),
							_List_fromArray(
								[
									$elm$html$Html$text(
									$elm$core$String$fromInt(
										$elm$core$List$length(statusTasks)))
								]))
						])),
					A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('kanban-cards')
						]),
					$elm$core$List$isEmpty(statusTasks) ? _List_fromArray(
						[
							A2(
							$elm$html$Html$div,
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('column-empty')
								]),
							_List_fromArray(
								[
									$elm$html$Html$text('등록된 업무가 없습니다')
								]))
						]) : A2(
						$elm$core$List$map,
						$author$project$Presentation$TaskBoard$taskCard(model),
						statusTasks))
				]));
	});
...
var $author$project$Presentation$TaskBoard$prioritySummary = function (tasks) {
	var summaryItem = function (quadrant) {
		...
		return A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class(
					'priority-summary-item ' + $author$project$Domain$Task$quadrantClass(quadrant))
				]),
			_List_fromArray(
				[
					A2(
	...
	return A2(
		...
		_List_fromArray(
			[
				summaryItem($author$project$Domain$Task$DoFirst),
				summaryItem($author$project$Domain$Task$Schedule),
				summaryItem($author$project$Domain$Task$Delegate),
				summaryItem($author$project$Domain$Task$Eliminate)
			]));
};
var $author$project$Presentation$TaskBoard$kanbanBoard = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('kanban-section')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('board-heading')
					]),
				_List_fromArray(
					[
						A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('empty board-empty')
					]),
				_List_fromArray(
					[
						$elm$html$Html$text('업무를 불러오는 중입니다…')
					])) : A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('kanban-board')
					]),
				A2(
					$elm$core$List$map,
					$author$project$Presentation$TaskBoard$kanbanColumn(model),
					$author$project$Domain$Task$allStatuses))
			]));
};
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'description',
				$elm$json$Json$Encode$string(input.description)),
				_Utils_Tuple2(
				'status',
				$elm$json$Json$Encode$string(
					$author$project$Domain$Task$statusString(input.status))),
				_Utils_Tuple2(
				'urgency',
				$elm$json$Json$Encode$string(
					$author$project$Domain$Task$urgencyString(input.urgency))),
				_Utils_Tuple2(
				'importance',
				$elm$json$Json$Encode$string(
					$author$project$Domain$Task$importanceString(input.importance))),
				_Utils_Tuple2(
				'taskOwner',
				$elm$json$Json$Encode$string(input.taskOwner)),
				_Utils_Tuple2(
				'outcomeOwner',
				$elm$json$Json$Encode$string(input.outcomeOwner)),
				_Utils_Tuple2(
				'expectedResult',
...
var $author$project$Infrastructure$TaskApi$perform = function (effect) {
	switch (effect.$) {
		case 'LoadTasks':
			return $elm$http$Http$get(
				{
					expect: A2(
						...
						A2(
							...
							$author$project$Application$TaskBoard$GotTasks,
							$elm$core$Result$mapError($author$project$Infrastructure$TaskApi$toApiError)),
						$elm$json$Json$Decode$list($author$project$Infrastructure$TaskApi$taskDecoder)),
					url: '/api/task'
				});
		case 'CreateTask':
			var input = effect.a;
			...
			return $elm$http$Http$request(
				{
					...
					expect: $elm$http$Http$expectWhatever(
						A2(
							$elm$core$Basics$composeL,
							$author$project$Application$TaskBoard$Deleted,
							$elm$core$Result$mapError($author$project$Infrastructure$TaskApi$toApiError))),
					headers: _List_Nil,
					method: 'DELETE',
					timeout: $elm$core$Maybe$Nothing,
...
var $author$project$Main$performEffect = function (effect) {
	if (effect.$ === 'ClearNoticeAfter') {
		var version = effect.a;
		return A2(
			$elm$core$Task$perform,
			function (_v1) {
				return $author$project$Application$TaskBoard$DismissNotice(version);
			},
			$elm$core$Process$sleep(3000));
	} else {
		return $author$project$Infrastructure$TaskApi$perform(effect);
	}
};
var $author$project$Main$withEffects = function (_v0) {
	var model = _v0.a;
	var effects = _v0.b;
	return _Utils_Tuple2(
		model,
		$elm$core$Platform$Cmd$batch(
			A2($elm$core$List$map, $author$project$Main$performEffect, effects)));
};
var $author$project$Main$main = $elm$browser$Browser$element(
	{
		init: function (_v0) {
			return $author$project$Main$withEffects($author$project$Application$TaskBoard$init);
		},
		...
		update: F2(
			function (message, model) {
				return $author$project$Main$withEffects(
					A2($author$project$Application$TaskBoard$update, message, model));
			}),
		view: $author$project$Presentation$TaskBoard$view
	});
