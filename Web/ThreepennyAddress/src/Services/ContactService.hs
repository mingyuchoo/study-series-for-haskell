...
class ContactService m where
    addContactM :: Contact -> AddressBookState -> m (Either [ValidationError] AddressBookState)
    updateContactM :: Contact -> AddressBookState -> m (Either [ValidationError] AddressBookState)
...
addContact contactData addressBook = do
    ...
    let newId = addressNextId addressBook
        ...
        updatedAddressBook = addressBook
