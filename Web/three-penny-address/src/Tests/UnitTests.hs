...
import           Services.ContactRepository (ContactRepository (..),
                                             FileContactRepository (..),
                                             loadContactsFromFile,
                                             saveContactsToFile)
import           Services.ContactService    (addContact, deleteContact,
                                             generateNextId, updateContact)
import           Services.SearchService     (SearchService (..), filterContacts,
                                             searchContacts)
import           Services.ValidationService (ValidationError (..),
                                             validateContactData, validateEmail,
                                             validatePhone)

import           System.Directory           (createDirectory, doesFileExist,
                                             getPermissions, removeDirectory,
                                             removeFile, setOwnerReadable,
                                             setPermissions,
...
validationTests = do
  ...
  describe "validateContactData" $ do
    it "accepts valid contact" $ do
      let contact =
            Contact (ContactId 1) "John Doe" (Just "010-1234-5678") (Just "john@example.com") Nothing
...
contactServiceTests = do
  let emptyState = emptyAddressBookState
      sampleContact =
        Contact (ContactId 0) "Test User" (Just "010-1234-5678") (Just "test@example.com") Nothing
    ...
    it "stores the generated contact id in the inserted contact" $ do
      ...
      case addContact sampleContact state of
        Right newState -> do
          ...
          fmap contactId (Map.lookup (ContactId 10) (addressContacts newState))
            `shouldBe` Just (ContactId 10)
...
repositoryTests = do
  let testFile = "test_contacts.json"
      testContacts =
        [ Contact
            ...
            "Test User 1"
            (Just "010-1111-1111")
            (Just "test1@example.com")
            Nothing
        , Contact
            (ContactId 2)
            "Test User 2"
            (Just "010-2222-2222")
            (Just "test2@example.com")
            Nothing
        ]
  ...
  describe "file operations" $ do
    ...
    it "exposes repository path and derived instances" $ do
      ...
      show repository
        `shouldBe` "FileContactRepository {repositoryFilePath = \"contacts.json\"}"
