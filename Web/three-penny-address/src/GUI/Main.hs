...
import           GUI.Components              (createButton, createContactRow,
                                              formatErrors)
import           GUI.Shortcuts               (KeyboardShortcut (..),
                                              handleKeyboardShortcut,
                                              parseKeyboardShortcut)
import           GUI.State                   (AppStateManager (..),
                                              FormMode (..),
                                              createAppStateManager,
...
import           Services.ContactRepository  (loadAddressBookFromFile,
...
setupGUI window = do
  ...
  emptyMessage <-
    UI.p
      # set text "No contacts found. Click 'Add Contact' to get started."
  ...
  void $
    element confirmDiv
      #+ [element confirmHeader, element confirmMessage, element confirmButtonDiv]
      ...
      Just cid -> do
        ...
        case Map.lookup cid (addressContacts $ appAddressBook appState) of
          ...
          Just contact -> do
            ...
            void $
              element addressInput # set UI.value (T.unpack $ fromMaybe "" $ contactAddress contact)
            void hideError
  ...
  shortcutNewBtn <-
    UI.button
      # set (UI.attr "id") "shortcut-new"
      # set UI.style [("display", "none")]
  shortcutDeleteBtn <-
    UI.button
      # set (UI.attr "id") "shortcut-delete"
      # set UI.style [("display", "none")]
  shortcutCancelBtn <-
    UI.button
      # set (UI.attr "id") "shortcut-cancel"
      # set UI.style [("display", "none")]
  shortcutSearchBtn <-
    UI.button
      # set (UI.attr "id") "shortcut-search"
      # set UI.style [("display", "none")]
