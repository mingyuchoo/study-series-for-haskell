...
import HaskellGPT.Types (Layer (..), Matrix, embeddingDim, maxSeqLen, xavierInit)
...
data Embeddings = Embeddings
  { embTokenEmbeddings      :: !(Matrix Float)
...
embedTokens emb tokenIds =
  let tokenEmb = embTokenEmbeddings emb
   ...
   in -- Stack embeddings into a matrix
      LA.fromRows embeddings
...
instance Layer Embeddings where
  ...
  forward emb input =
    let
      -- Input is expected to be a matrix of token IDs (batch_size x seq_len)
  ...
  backward emb grads lr =
    case embCachedInput emb of
      ...
      Just input ->
        let
          -- Extract token IDs from cached input
          tokenIds = map round $ LA.toList $ LA.flatten input
          seqLen = length tokenIds
